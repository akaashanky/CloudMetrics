...
public:					
	...
	// Connection handler	
	BOOL Connect(LPCTSTR szAddress, LPCTSTR szAgent = _T(__DEFAULT_AGENT_NAME), unsigned short nPort = INTERNET_DEFAULT_HTTP_PORT, LPCTSTR szUserAccount = NULL, LPCTSTR szPassword = NULL);
	BOOL Close();
	...
	// HTTP Method handler 
	BOOL Request(LPCTSTR szURL, int nMethod = GenericHTTPClient::RequestGetMethod, LPCTSTR szAgent = _T(__DEFAULT_AGENT_NAME));
