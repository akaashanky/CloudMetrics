...
BOOL GenericHTTPClient::RequestGet(LPCTSTR szURI){
	...
	CONST TCHAR *szAcceptType= _T(__HTTP_ACCEPT_TYPE);

	_hHTTPRequest=::HttpOpenRequest(	_hHTTPConnection,
															_T(__HTTP_VERB_GET), // HTTP Verb
															szURI, // Object Name
	...
	// REPLACE HEADER
	if(!::HttpAddRequestHeaders( _hHTTPRequest, _T(__HTTP_ACCEPT), _tcslen(_T(__HTTP_ACCEPT)), HTTP_ADDREQ_FLAG_REPLACE)){
		_dwError=::GetLastError();
#ifdef	_DEBUG
		LPVOID     lpMsgBuffer;
		DWORD dwRet=FormatMessage( FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM,
